...
class DjsSpider(scrapy.Spider):
    ...
    def parse_detail(self, response):
        ...
        row = {
            ...
            'ETD': GetMiddleStr(table.find('tr:nth-child(4) > td:nth-child(2)').text(), '\(', '\)'),
            'VESSEL': vv[0],
            'VOYAGE': vv[1],
            'ETA': '',
            'TRANSIT_TIME': 0,  # 无
        ...
        if step_num > 1:
            ...
            row['ETA'] = GetMiddleStr(table.find('tr:nth-child(4) > td:nth-child(4)').text(), '\(', '\)')
        logging.info(row)
